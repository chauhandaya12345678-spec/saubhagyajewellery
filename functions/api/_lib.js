...
export async function pushToShipPrime(env, order, db) {
  if (env.UAT_MODE === 'true') return { pushed: true, mock: true, awb: null, shipPrimeOrderId: 'UAT-MOCK', reason: 'UAT sandbox — ShipPrime push simulated (nothing sent)' };
  try {
...
export async function sendWhatsAppMessage(env, toPhone, templateName, params, headerImageUrl) {
  if (env.UAT_MODE === 'true') return { sent: true, mock: true, msgId: 'UAT-MOCK/' + templateName, preview: { template: templateName, to: '91' + String(toPhone || '').replace(/\D/g, '').slice(-10), params: params || [] }, reason: 'UAT sandbox — WhatsApp simulated (not sent to any real number)' };
  try {
...
export async function sendOrderEmail(env, order) {
  if (env.UAT_MODE === 'true') return { sent: true, mock: true, id: 'UAT-MOCK', preview: { to: order && order.email, subject: 'order confirmation' }, reason: 'UAT sandbox — email simulated (not sent)' };
  try {
